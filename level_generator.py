...
class GridGenerator:
  ...
  def Print(self) -> None:
    lookup = [Fore.RED, Fore.YELLOW, Fore.GREEN, Fore.CYAN, Fore.BLUE, Fore.MAGENTA]
    lookup2 = [Fore.RED, Fore.BLUE, Fore.YELLOW]
    random.shuffle(lookup)
    random.shuffle(lookup2)
    lookup.insert(0, Fore.WHITE)
    lookup.extend(lookup2)
    for row in range(0, 0x8):
      for col in range(0, 0x10):
        level_num = int(self.grid[0x10 * row + col])
        print(lookup[level_num] + str(level_num), end="")
      print("")
    print(Fore.CYAN)
...
class LevelGenerator:
  ...
  def RandomizeOverworldCaves(self) -> None:
    foo: List[int] = []
    for screen_num in Screen.ALL_SCREENS:
      foo.append(self.data_table.GetCaveDestination(screen_num))
    random.shuffle(foo)
    for screen_num in Screen.ALL_SCREENS:
      self.data_table.SetCaveDestination(screen_num, foo.pop())
  ...
  def RandomizeStuff(self) -> None:
    ...
    elder_assignments.append([Enemy.ELDER])  #, Enemy.ELDER_2])  # Level 9
...
#    elder_group_b.append(Enemy.ELDER_6)
    ...
    while not done:
      ...
      if elder_group_a[0] != Enemy.BOMB_UPGRADER or elder_group_b[1] != Enemy.BOMB_UPGRADER:
        done = True
    #assert len(elder_group_a) == 6 and len(elder_group_b) == 6
    assert len(elder_group_a) == 4 and len(elder_group_b) == 4
    for n in range(0, 4):
    ...
    for level_num in Range.VALID_LEVEL_NUMBERS:
      ...
      while not done:
        try:
          ...
          self.AddRoomTypes(level_num, item_stairway_assignments[level_num],
...
#          sys.exit()
  ...
  def AddRoomTypes(self, level_num: LevelNum, item_staircase_room_list: List[RoomNum],
                   transport_staircase_room_list: List[RoomNum]) -> None:
      ...
      for room_num in room_nums:
        ...
        while not all_room_type_checks_passed:
          ...
          if room_type == RoomType.ELDER_ROOM and room.HasShutterDoor():
            ...
            continue
          if room_type == RoomType.ELDER_ROOM and room.GetWallType(
